...
fn discover_definition_roots(cwd: &Path, leaf: &str) -> Vec<(DefinitionSource, PathBuf)> {
    ...
    for ancestor in cwd.ancestors() {
        ...
        push_unique_root(
            ...
            ancestor.join(".mycli-legacy").join(leaf),
        );
    }
    ...
    if let Some(home) = env::var_os("HOME") {
        ...
        push_unique_root(
            &mut roots,
            DefinitionSource::UserClaude,
            home.join(".mycli-legacy").join(leaf),
        );
    }
...
fn discover_skill_roots(cwd: &Path) -> Vec<SkillRoot> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectClaw,
            ancestor.join(".mycli").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectClaude,
            ancestor.join(".mycli-legacy").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectClaw,
            ancestor.join(".mycli").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
        push_unique_skill_root(
            &mut roots,
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectClaude,
            ancestor.join(".mycli-legacy").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
    }
    ...
    if let Some(home) = env::var_os("HOME") {
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserClaude,
            home.join(".mycli-legacy").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserClaude,
            home.join(".mycli-legacy").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
    }
...
fn default_skill_install_root() -> std::io::Result<PathBuf> {
    ...
    if let Some(home) = env::var_os("HOME") {
        return Ok(PathBuf::from(home).join(".mycli").join("skills"));
    }
    Err(std::io::Error::new(
        std::io::ErrorKind::NotFound,
...
mod tests {
    ...
    fn lists_agents_from_project_and_user_roots() {
        ...
        let user_agents = user_home.join(".mycli-legacy").join("agents");
    ...
    fn lists_skills_from_project_and_user_roots() {
        ...
        let project_commands = workspace.join(".mycli-legacy").join("commands");
    ...
    fn resolves_project_skills_and_legacy_commands_from_shared_registry() {
        ...
        let project_skills = workspace.join(".mycli").join("skills");
        let legacy_commands = workspace.join(".mycli").join("commands");
