...
const OFFICIAL_REPO_URL: &str = "https://github.com/my-org/my-cli";
const OFFICIAL_REPO_SLUG: &str = "my-org/my-cli";
const DEPRECATED_INSTALL_COMMAND: &str = "cargo install my-cli";
...
fn main() {
    if let Err(error) = run() {
        ...
        } else if message.contains("`my-cli --help`") {
            eprintln!("error: {message}");
        } else {
            eprintln!(
                "error: {message}

Run `my-cli --help` for usage."
            );
        }
        std::process::exit(1);
    }
    ...
    while index < args.len() {
        match args[index].as_str() {
                    ...
                    ) =>
            {
                ...
                // the arg to the API (e.g. `my-cli prompt --help`) should show
            ...
            "-p" => {
                // My CLI compat: -p "prompt" = one-shot prompt
            ...
            "--print" => {
                // My CLI compat: --print makes output non-interactive
...
fn parse_acp_args(args: &[String], output_format: CliOutputFormat) -> Result<CliAction, String> {
    match args {
        ...
        _ => Err(String::from(
            "unsupported ACP invocation. Use `my-cli acp`, `my-cli acp serve`, `my-cli --acp`, or `my-cli -acp`.",
        )),
    }
}
...
fn omc_compatibility_note_for_unknown_slash_command(name: &str) -> Option<&'static str> {
    name.starts_with("oh-my-claudecode:")
        .then_some(
            "Compatibility note: `/oh-my-claudecode:*` is a My CLI/OMC plugin command. `claw` does not yet load plugin slash commands, Claude statusline stdin, or OMC session hooks.",
        )
}
...
/// Read `.mycli/worker-state.json` from the current working directory and print it.
...
const DUMP_MANIFESTS_OVERRIDE_HINT: &str =
    "Hint: set CLAUDE_CODE_UPSTREAM=/path/to/upstream or pass `my-cli dump-manifests --manifests-dir /path/to/upstream`.";
...
fn render_resume_usage() -> String {
  ...
  Auto-save        .mycli/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}
 ...
 ╚═════╝╚══════╝╚═╝  ╚═╝ ╚══╝╚══╝\x1b[0m \x1b[38;5;208mCode\x1b[0m 🦞\n\n\
  ...
  Type \x1b[1m/help\x1b[0m for commands · \x1b[1m/status\x1b[0m for live context · \x1b[2m/resume latest\x1b[0m jumps back to the newest session · \x1b[1m/diff\x1b[0m then \x1b[1m/commit\x1b[0m to ship · \x1b[2mTab\x1b[0m for workflow completions · \x1b[2mShift+Enter\x1b[0m for newline",
    ...
    fn print_mcp(
        ...
        // `my-cli mcp serve` starts a stdio MCP server exposing claw's built-in
...
fn render_repl_help() -> String {
    [
        ...
        "  Auto-save            .mycli/sessions/<session-id>.jsonl".to_string(),
    ...
    json!({
        ...
        "workspace": {
            ...
            "session_id": context.session_path.as_ref().and_then(|path| {
                ...
                // .mycli/sessions/. Extract the stem (drop the .jsonl extension).
...
fn print_acp_status(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    let message = "ACP/Zed editor integration is not implemented in claw-code yet. `my-cli acp serve` is only a discoverability alias today; it does not launch a daemon or Zed-specific protocol endpoint. Use the normal terminal surfaces for now and track ROADMAP #76 for real ACP support.";
    match output_format {
        CliOutputFormat::Text => {
            println!(
                "ACP / Zed\n  Status           discoverability only\n  Launch           `my-cli acp serve` / `my-cli --acp` / `my-cli -acp` report status only; no editor daemon is available yet\n  Today            use `my-cli prompt`, the REPL, or `my-cli doctor` for local verification\n  Tracking         ROADMAP #76\n  Message          {message}"
            );
        }
        CliOutputFormat::Json => {
            println!(
...
impl AnthropicRuntimeClient {
    fn new(
        ...
        // (`crates/my-cli-cli/tests/compact_output.rs`) to point
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    ...
    writeln!(
        out,
        "  REPL turns auto-save to .mycli/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}"
    )?;
    writeln!(
        out,
...
mod tests {
    ...
    fn temp_dir() -> PathBuf {
        ...
        std::env::temp_dir().join(format!("my-cli-cli-{nanos}-{unique}"))
    }
    ...
    fn formats_namespaced_omc_slash_command_with_contract_guidance() {
        ...
        assert!(report.contains("My CLI/OMC plugin command"));
    ...
    fn repl_help_includes_shared_commands_and_exit() {
        ...
        assert!(help.contains("Auto-save            .mycli/sessions/<session-id>.jsonl"));
...
UU conflicted.rs",
    ...
    fn managed_sessions_default_to_jsonl_and_resolve_legacy_json() {
        ...
        let legacy_path = workspace.join(".mycli/sessions/legacy.json");
    ...
    fn load_session_reference_rejects_workspace_mismatch() {
        ...
        let session_path = workspace_a.join(".mycli/sessions/legacy-cross.jsonl");
        std::fs::create_dir_all(
            session_path
                .parent()
    ...
    fn unknown_omc_slash_command_guidance_explains_runtime_gap() {
        ...
        assert!(message.contains("My CLI/OMC plugin command"));
    ...
    fn resume_usage_mentions_latest_shortcut() {
        ...
        assert!(usage.contains(".mycli/sessions/<session-id>.jsonl"));
    ...
    fn init_template_mentions_detected_rust_workspace() {
        ...
        let rendered = crate::init::render_init_mycli_md(&workspace_root);
        assert!(rendered.contains("# MYCLI.md"));
    ...
    fn ultraplan_progress_lines_include_phase_step_and_elapsed_status() {
        let snapshot = InternalPromptProgressState {
            ...
            detail: Some("reading rust/crates/my-cli-cli/src/main.rs".to_string()),
            saw_final_text: false,
        };
    ...
    fn describe_tool_progress_summarizes_known_tools() {
        ...
        assert!(
            describe_tool_progress("bash", r#"{"command":"cargo test -p my-cli-cli"}"#)
                .contains("cargo test -p my-cli-cli")
        );
        assert_eq!(
