...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let claude_md = cwd.join("MYCLI.md");
    let content = render_init_mycli_md(cwd);
    artifacts.push(InitArtifact {
        name: "MYCLI.md",
...
pub(crate) fn render_init_mycli_md(cwd: &Path) -> String {
    let detection = detect_repo(cwd);
    let mut lines = vec![
        "# MYCLI.md".to_string(),
        String::new(),
        "This file provides guidance to My CLI (my-cli.dev) when working with code in this repository.".to_string(),
        String::new(),
    ];
...
#[cfg(test)]
mod tests {
    use super::{initialize_repo, render_init_mycli_md};
    use std::fs;
    use std::path::Path;
    ...
    fn temp_dir() -> std::path::PathBuf {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("my-cli-init-{nanos}"))
    }
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        assert!(first
            .render()
            .contains("MYCLI.md        skipped (already exists)"));
        ...
        assert_eq!(
            fs::read_to_string(root.join("MYCLI.md")).expect("read existing claude md"),
            "custom guidance\n"
        );
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
    ...
    fn render_init_template_mentions_detected_python_and_nextjs_markers() {
        ...
        let rendered = render_init_mycli_md(Path::new(&root));
