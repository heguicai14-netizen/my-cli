...
fn push_project_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, cwd: &std::path::Path) {
    for ancestor in cwd.ancestors() {
        ...
        push_prefixed_skill_lookup_roots(roots, &ancestor.join(".mycli"));
        push_prefixed_skill_lookup_roots(roots, &ancestor.join(".codex"));
        push_prefixed_skill_lookup_roots(roots, &ancestor.join(".mycli-legacy"));
    }
}
...
fn push_home_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, home: &std::path::Path) {
    ...
    push_prefixed_skill_lookup_roots(roots, &home.join(".mycli"));
    push_prefixed_skill_lookup_roots(roots, &home.join(".codex"));
    push_prefixed_skill_lookup_roots(roots, &home.join(".mycli-legacy"));
    push_skill_lookup_root(
        roots,
...
fn agent_store_dir() -> Result<std::path::PathBuf, String> {
    ...
    if let Some(workspace_root) = cwd.ancestors().nth(2) {
        return Ok(workspace_root.join(".mycli-agents"));
    }
    Ok(cwd.join(".mycli-agents"))
}
...
mod tests {
    ...
    fn worker_create_merges_config_trusted_roots_without_per_call_override() {
        ...
        let claw_dir = worktree.join(".mycli");
    ...
    fn recovery_loop_state_file_reflects_transitions() {
        // End-to-end proof: .mycli/worker-state.json reflects every transition
        ...
        let state_path = worktree.join(".mycli").join("worker-state.json");
    ...
    fn skill_resolves_project_local_skills_and_legacy_commands() {
        ...
        let skill_dir = root.join(".mycli").join("skills").join("plan");
        let command_dir = root.join(".mycli").join("commands");
        fs::create_dir_all(&skill_dir).expect("skill dir should exist");
    ...
    fn skill_loads_project_local_claude_skill_prompt() {
        ...
        let skill_dir = workspace.join(".mycli-legacy").join("skills").join("trace");
        fs::create_dir_all(&skill_dir).expect("skill dir should exist");
    ...
    fn skill_loads_project_local_legacy_command_markdown() {
        ...
        let workspace = root.join("workspace");
        let nested = workspace.join("nested");
        let command_dir = workspace.join(".mycli-legacy").join("commands");
  ...
  "nbformat_minor": 5
    ...
    fn config_reads_and_writes_supported_values() {
        ...
        std::fs::write(
            home.join(".mycli").join("settings.json"),
            r#"{"verbose":false}"#,
        )
    ...
    fn enter_and_exit_plan_mode_round_trip_existing_local_override() {
        ...
        let state =
            std::fs::read_to_string(cwd.join(".mycli").join("tool-state").join("plan-mode.json"))
