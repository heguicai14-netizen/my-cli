...
function isPathAllowed(
  ...
  // This MUST come before checkPathSafetyForAutoEdit since .mycli is a dangerous directory
  // and internal editable paths live under ~/.mycli/ — matching the ordering in
...
function checkPathConstraintsForStatement(
  ...
  //   Set-Location ./.mycli; Set-Content ./settings.json '...'
  // Validator sees ./settings.json → /project/settings.json (not a config file).
  // Runtime writes /project/.mycli/settings.json (Claude's permission config).
  //
  // ALTERNATIVE APPROACH (rejected): simulate cwd through the statement chain
  // — after `Set-Location ./.mycli`, validate subsequent statements with
  // cwd='./.mycli'. This would be more permissive but requires careful
  // handling of:
