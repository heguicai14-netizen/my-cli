 ...
 *
 * 1. Managed memory (eg. /etc/claude-code/MYCLI.md) - Global instructions for all users
 * 2. User memory (~/.mycli/MYCLI.md) - Private global instructions for all projects
 * 3. Project memory (MYCLI.md, .mycli/MYCLI.md, and .mycli/rules/*.md in project roots) - Instructions checked into the codebase
 * 4. Local memory (MYCLI.local.md in project roots) - Private project-specific instructions
 *
 ...
 * - MYCLI.md, .mycli/MYCLI.md, and all .md files in .mycli/rules/ are checked in each directory for Project memory
 *
...
/**
 * Checks whether a MYCLI.md file path is excluded by the claudeMdExcludes setting.
...
function isClaudeMdExcluded(filePath: string, type: MemoryType): boolean {
  ...
  // the user writes "/tmp/project/MYCLI.md" in their exclude, but the system
...
/**
 * Processes all .md files in the .mycli/rules/ directory and its subdirectories
...
export const getMemoryFiles = memoize(
  async (forceIncludeExternal: boolean = false): Promise<MemoryFileInfo[]> => {
    ...
    // Process Managed .mycli/rules/*.md files
    ...
    if (isSettingSourceEnabled('userSettings')) {
      ...
      // Process User ~/.mycli/rules/*.md files
    ...
    // .mycli/worktrees/<name>/ from `claude -w`), the upward walk passes
    // through both the worktree root and the main repo root. Both contain
    // checked-in files like MYCLI.md and .mycli/rules/*.md, so the same
    // content gets loaded twice. Skip Project-type (checked-in) files from
    // directories above the worktree but within the main repo — the worktree
    // already has its own checkout. MYCLI.local.md is gitignored so it only
    ...
    for (const dir of dirs.reverse()) {
      ...
      // Try reading MYCLI.md (Project) - only if projectSettings is enabled
      if (isSettingSourceEnabled('projectSettings') && !skipProject) {
        const projectPath = join(dir, 'MYCLI.md')
        result.push(
          ...(await processMemoryFile(
            projectPath,
            'Project',
        ...
        // Try reading .mycli/MYCLI.md (Project)
        const dotClaudePath = join(dir, '.mycli', 'MYCLI.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
            'Project',
        ...
        // Try reading .mycli/rules/*.md files (Project)
        const rulesDir = join(dir, '.mycli', 'rules')
        result.push(
          ...(await processMdRules({
            rulesDir,
            type: 'Project',
      ...
      // Try reading MYCLI.local.md (Local) - only if localSettings is enabled
      if (isSettingSourceEnabled('localSettings')) {
        const localPath = join(dir, 'MYCLI.local.md')
        result.push(
          ...(await processMemoryFile(
            localPath,
            'Local',
    ...
    // Process MYCLI.md from additional directories (--add-dir) if env var is enabled
    ...
    if (isEnvTruthy(process.env.CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD)) {
      ...
      for (const dir of additionalDirs) {
        // Try reading MYCLI.md from the additional directory
        const projectPath = join(dir, 'MYCLI.md')
        result.push(
          ...(await processMemoryFile(
            projectPath,
        ...
        // Try reading .mycli/MYCLI.md from the additional directory
        const dotClaudePath = join(dir, '.mycli', 'MYCLI.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
        ...
        // Try reading .mycli/rules/*.md files from the additional directory
        const rulesDir = join(dir, '.mycli', 'rules')
        result.push(
          ...(await processMdRules({
            rulesDir,
    ...
    // memory system, not "instructions" in the MYCLI.md/rules sense.
...
export async function getManagedAndUserConditionalRules(
  ...
  // Process Managed conditional .mycli/rules/*.md files
  const managedClaudeRulesDir = getManagedClaudeRulesDir()
  result.push(
  ...
  if (isSettingSourceEnabled('userSettings')) {
    // Process User conditional .mycli/rules/*.md files
    const userClaudeRulesDir = getUserClaudeRulesDir()
    result.push(
      ...(await processConditionedMdRules(
        targetPath,
 ...
 * Loads MYCLI.md, unconditional rules, and conditional rules for that directory.
 *
...
export async function getMemoryFilesForNestedDirectory(
  ...
  // Process project memory files (MYCLI.md and .mycli/MYCLI.md)
  ...
  // Process local memory file (MYCLI.local.md)
  if (isSettingSourceEnabled('localSettings')) {
    const localPath = join(dir, 'MYCLI.local.md')
    result.push(
  ...
  // Process project unconditional .mycli/rules/*.md files, which were not eagerly loaded
  ...
  // Process project conditional .mycli/rules/*.md files
  result.push(
    ...(await processConditionedMdRules(
      targetPath,
      rulesDir,
...
export async function processConditionedMdRules(
  ...
  return conditionedRuleMdFiles.filter(file => {
    ...
    // For Project rules: glob patterns are relative to the directory containing .mycli
    // For Managed/User rules: glob patterns are relative to the original CWD
    const baseDir =
      type === 'Project'
        ? dirname(dirname(rulesDir)) // Parent of .mycli
...
/**
 * Check if a file path is a memory file (MYCLI.md, MYCLI.local.md, or .mycli/rules/*.md)
 */
export function isMemoryFilePath(filePath: string): boolean {
  const name = basename(filePath)

  // MYCLI.md or MYCLI.local.md anywhere
  if (name === 'MYCLI.md' || name === 'MYCLI.local.md') {
    return true
  }

  // .md files in .mycli/rules/ directories
  if (
    name.endsWith('.md') &&
    filePath.includes(`${sep}.mycli${sep}rules${sep}`)
  ) {
    return true
  }
