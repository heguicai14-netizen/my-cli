// Scheduler lease lock for .mycli/scheduled_tasks.json.
//
...
const LOCK_FILE_REL = join('.mycli', 'scheduled_tasks.lock')
...
async function tryCreateExclusive(
  ...
  try {
    ...
    if (code === 'ENOENT') {
      // .mycli/ doesn't exist yet — create it and retry once. In steady
