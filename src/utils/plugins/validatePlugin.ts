...
function detectManifestType(
  ...
  // Check if it's in .mycli-plugin directory
  if (dirName === '.mycli-plugin') {
 ...
 * '..' a user added to "climb out of .mycli-plugin/" is unnecessary. Callers pass `hint`
...
// they expect paths to resolve relative to marketplace.json (inside .mycli-plugin/),
...
function marketplaceSourceHint(p: string): string {
  ...
  // .mycli-plugin/" is unnecessary since paths already start at the repo root.
  ...
  return (
    ...
    'containing .mycli-plugin/), not relative to marketplace.json. ' +
  ...
  if (result.success) {
    ...
    if (marketplace.plugins) {
      ...
      const marketplaceRoot =
        path.basename(manifestDir) === '.mycli-plugin'
      ...
      for (const [i, entry] of marketplace.plugins.entries()) {
        ...
        if (manifestVersion && manifestVersion !== entry.version) {
          warnings.push({
            ...
            message:
              `Entry declares version "${entry.version}" but ${entry.source}/.mycli-plugin/plugin.json says "${manifestVersion}". ` +
...
export async function validateManifest(
  ...
  if (stats?.isDirectory()) {
    // Look for manifest files in .mycli-plugin directory
    ...
    const pluginPath = path.join(absolutePath, '.mycli-plugin', 'plugin.json')
    ...
    return {
      ...
      errors: [
        {
          path: 'directory',
          message: `No manifest found in directory. Expected .mycli-plugin/marketplace.json or .mycli-plugin/plugin.json`,
        },
      ],
      warnings: [],
      filePath: absolutePath,
