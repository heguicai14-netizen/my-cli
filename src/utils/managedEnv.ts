 ...
 * env vars that the remote's ~/.mycli settings.env MUST NOT clobber (see
 ...
 * user's ~/.mycli/settings.json can't redirect requests away from the
 ...
 *
 * - userSettings (~/.mycli/settings.json): controlled by the user, not project-specific
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // Global config (~/.mycli.json) is user-controlled. In CCD mode,
  ...
  // doesn't get clobbered by ~/.mycli/settings.json env (gh#217). policy/flag
