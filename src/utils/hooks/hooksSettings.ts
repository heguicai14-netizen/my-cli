...
export function getAllHooks(appState: AppState): IndividualHookConfig[] {
  ...
  if (!restrictedToManagedOnly) {
    ...
    // both resolve to ~/.mycli/settings.json)
...
export function hookSourceDescriptionDisplayString(source: HookSource): string {
  switch (source) {
    case 'userSettings':
      return 'User settings (~/.mycli/settings.json)'
    case 'projectSettings':
      return 'Project settings (.mycli/settings.json)'
    case 'localSettings':
      return 'Local settings (.mycli/settings.local.json)'
    case 'pluginHook':
      ...
      // e.g., "Plugin hooks (~/.mycli/plugins/repos/source/example-plugin/example-plugin/hooks/hooks.json)"
      return 'Plugin hooks (~/.mycli/plugins/*/hooks/hooks.json)'
    case 'sessionHook':
