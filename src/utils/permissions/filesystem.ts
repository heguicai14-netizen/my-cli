...
/**
 * If filePath is inside a .mycli/skills/{name}/ directory (project or global),
 ...
 * require granting session access to all of .mycli/ (settings.json, hooks/, etc.).
 */
export function getClaudeSkillScope(
  ...
  const bases = [
    {
      dir: expandPath(join(getOriginalCwd(), '.mycli', 'skills')),
      prefix: '/.mycli/skills/',
    },
    {
      dir: expandPath(join(homedir(), '.mycli', 'skills')),
      prefix: '~/.mycli/skills/',
    },
  ]
  ...
  for (const { dir, prefix } of bases) {
    ...
    for (const s of [sep, '/']) {
      if (absolutePathLower.startsWith(dirLower + s.toLowerCase())) {
        ...
        // produce '/.mycli/skills/*/**' which matches ALL skills. Return null
...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  // sequences like `./.mycli/./settings.json` which would evade the endsWith() check
  ...
  if (
    normalizedPath.endsWith(`${sep}.mycli${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.mycli${sep}settings.local.json`)
  ) {
    // Include .mycli/settings.json even for other projects
    return true
  }
...
function isClaudeConfigFilePath(filePath: string): boolean {
  ...
  // Check if file is within .mycli/commands or .mycli/agents directories
  // using proper path segment validation (not string matching with includes())
  // pathInWorkingPath now handles case-insensitive comparison to prevent bypasses
  const commandsDir = join(getOriginalCwd(), '.mycli', 'commands')
  const agentsDir = join(getOriginalCwd(), '.mycli', 'agents')
  const skillsDir = join(getOriginalCwd(), '.mycli', 'skills')
 ...
 * Path format: ~/.mycli/projects/{sanitized-cwd}/...
 */
...
function isDangerousFilePathToAutoEdit(path: string): boolean {
  ...
  for (let i = 0; i < pathSegments.length; i++) {
    ...
    for (const dir of DANGEROUS_DIRECTORIES) {
      ...
      // Special case: .mycli/worktrees/ is a structural path (where Claude stores
      // git worktrees), not a user-created dangerous directory. Skip the .mycli
      // segment when it's followed by 'worktrees'. Any nested .mycli directories
      ...
      if (dir === '.mycli') {
        ...
        if (
          ...
          break // Skip this .mycli, continue checking other segments
        }
      }
 ...
 * - Trailing dots and spaces (e.g., .git., .mycli , .bashrc...)
...
function hasSuspiciousWindowsPathPattern(path: string): boolean {
  ...
  // Examples: .git., .mycli , .bashrc..., settings.json.
 ...
 * - Claude config files (.mycli/settings.json, .mycli/commands/, .mycli/agents/)
...
function patternWithRoot(
  ...
  if (pattern.startsWith(`${DIR_SEP}${DIR_SEP}`)) {
    ...
    // Patterns starting with / resolve relative to the directory where settings are stored (without .mycli/)
    return {
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  // This MUST come before isDangerousFilePathToAutoEdit check since .mycli is a dangerous directory
  ...
  // 1.6. Check for .mycli/** allow rules BEFORE safety checks
  // This allows session-level permissions to bypass the safety blocks for .mycli/
  // We only allow this for session-level rules to prevent users from accidentally
  // permanently granting broad access to their .mycli/ folder.
  //
  // matchingRuleForInput returns the first match across all sources. If the user
  // also has a broader Edit(.mycli) rule in userSettings (e.g. from sandbox
  ...
  if (claudeFolderAllowRule) {
    // Check if this rule is scoped under .mycli/ (project or global).
    // Accepts both the broad patterns ('/.mycli/**', '~/.mycli/**') and
    // narrowed ones like '/.mycli/skills/my-skill/**' so users can grant
    ...
    // '/.mycli/../**' from leaking this bypass outside .mycli/.
  ...
  if (!safetyCheck.safe) {
    // SDK suggestion: if under .mycli/skills/{name}/, emit the narrowed
    // session-scoped addRules that step 1.6 will honor on the next call.
    // Everything else (.mycli/settings.json, .git/, .vscode/, .idea/) falls
...
export function checkEditableInternalPath(
  ...
  // var value must itself resolve under ~/.mycli/jobs/. Symlink guard: every
  ...
  if (feature('TEMPLATES')) {
    ...
    if (jobDir) {
      ...
      // the case where ~/.mycli is a symlink (e.g. to /data/claude-config).
  ...
  // ~/.mycli/, which is in DANGEROUS_DIRECTORIES. The CLAUDE_COWORK_MEMORY_PATH_OVERRIDE
  ...
  // .mycli/launch.json — desktop preview config (dev server command + port).
  ...
  // .mycli/ DANGEROUS_DIRECTORIES check prompts for it, which in SDK mode
  ...
  // .mycli/ only (not ~/.mycli/) since launch.json is per-project.
  if (
    normalizeCaseForComparison(normalizedPath) ===
    normalizeCaseForComparison(join(getOriginalCwd(), '.mycli', 'launch.json'))
  ) {
    return {
      behavior: 'allow',
      updatedInput: input,
...
export function checkReadableInternalPath(
  ...
  // Tasks directory (~/.mycli/tasks/) for swarm task coordination
  ...
  // Teams directory (~/.mycli/teams/) for swarm coordination
