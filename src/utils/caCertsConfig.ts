 ...
 * user-controlled files (~/.mycli/settings.json and ~/.mycli.json),
 ...
 *
 * We read from global config (~/.mycli.json) and user settings
 * (~/.mycli/settings.json). These are user-controlled files that don't
...
function getExtraCertsPathFromConfig(): string | undefined {
  try {
    ...
    // Only read from user-controlled settings (~/.mycli/settings.json),
