 ...
 * notice which directory — and therefore which MYCLI.md — was loaded.
 */
 ...
 *
 * Always shows the working directory so the user can see which MYCLI.md
 ...
 * and whether its MYCLI.md may be stale relative to upstream.
 */
export function buildDeepLinkBanner(info: DeepLinkBannerInfo): string {
  ...
  if (info.repo) {
    ...
    lines.push(
      `Resolved ${info.repo} from local clones · last fetched ${age}${stale ? ' — MYCLI.md may be stale' : ''}`,
    )
  }
