 ...
 * stopping there makes the parent project's `.mycli/` unreachable (#31905).
 *
 ...
 *
 * Worktrees (under `.mycli/worktrees/`) stay on the old behavior: their `.git`
 ...
 * collecting all .mycli directories along the way.
 *
 * Stopping at git root prevents commands/skills from parent directories outside the repository
 * from leaking into projects. For example, if ~/projects/.mycli/commands/ exists, it won't
 ...
 * @returns Array of directory paths containing .mycli/subdir, from most specific (cwd) to least specific
 */
export function getProjectDirsUpToHome(
  ...
  while (true) {
    ...
    const claudeSubdir = join(current, '.mycli', subdir)
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    const managedDir = join(getManagedFilePath(), '.mycli', subdir)
    const projectDirs = getProjectDirsUpToHome(subdir, cwd)

    // For git worktrees where the worktree does NOT have .mycli/<subdir> checked
    ...
    //
    // Only add the main repo's copy when the worktree root's .mycli/<subdir>
    // is absent. A standard `git worktree add` checks out the full tree, so the
    // worktree already has identical .mycli/<subdir> content — loading the main
    ...
    // This prevents the same file from appearing multiple times when ~/.mycli is
 ...
 * @param dir Directory (eg. "~/.mycli/commands")
 * @returns Array of parsed markdown files with metadata
 */
