...
export function convertToSandboxRuntimeConfig(
  ...
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.mycli', 'settings.json'))
    denyWrite.push(resolve(cwd, '.mycli', 'settings.local.json'))
  }

  // Block writes to .mycli/skills in both original and current working directories.
  // The sandbox-runtime's getDangerousDirectories() protects .mycli/commands and
  // .mycli/agents but not .mycli/skills. Skills have the same privilege level
  // (auto-discovered, auto-loaded, full Claude capabilities) so they need the
  // same OS-level sandbox protection.
  denyWrite.push(resolve(originalCwd, '.mycli', 'skills'))
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.mycli', 'skills'))
  }
