...
import { getMemoryFiles, type MemoryFileInfo } from '../../utils/myclimd.js';
...
export function MemoryFileSelector(t0) {
  ...
  const userMemoryPath = join(getClaudeConfigHomeDir(), "MYCLI.md");
  const projectMemoryPath = join(getOriginalCwd(), "MYCLI.md");
  ...
  const memoryOptions = allMemoryFiles.map(file => {
    ...
    if (file.type === "User" && !file.isNested) {
      description = "Saved in ~/.mycli/MYCLI.md";
    } else {
      if (file.type === "Project" && !file.isNested && file.path === projectMemoryPath) {
        description = `${isGit ? "Checked in at" : "Saved in"} ./MYCLI.md`;
      } else {
        if (file.parent) {
