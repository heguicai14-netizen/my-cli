...
async function main(): Promise<void> {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.VERSION} (MY CLI)`);
    return;
  }
  ...
  if (process.argv[2] === '--claude-in-chrome-mcp') {
    ...
    } = await import('../utils/mycliInChrome/mcpServer.js');
    ...
    const {
      runChromeNativeHost
    } = await import('../utils/mycliInChrome/chromeNativeHost.js');
  ...
  // Session management against the ~/.mycli/sessions/ registry. Flag
