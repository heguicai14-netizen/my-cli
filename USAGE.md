...
This guide covers the current Rust workspace under `rust/` and the `my-cli` CLI binary. If you are brand new, make the doctor health check your first run: start `my-cli`, then run `/doctor`.
...
`/doctor` is the built-in setup and preflight diagnostic. Once you have a saved session, you can rerun it with `./target/debug/my-cli --resume latest /doctor`.
...
The CLI binary is available at `rust/target/debug/my-cli` after a debug build. Make the doctor check above your first post-build step.
...
```bash
cd rust
./target/debug/my-cli doctor --output-format json
```
...
```bash
cd rust
./target/debug/my-cli prompt "summarize this repository"
```
...
```bash
cd rust
./target/debug/my-cli "explain rust/crates/runtime/src/lib.rs"
```
...
```bash
cd rust
./target/debug/my-cli --model sonnet prompt "review this diff"
./target/debug/my-cli --permission-mode read-only prompt "summarize Cargo.toml"
./target/debug/my-cli --permission-mode workspace-write prompt "update README.md"
./target/debug/my-cli --allowedTools read,glob "inspect the runtime crate"
```
...
**Why this matters:** if you paste an `sk-ant-*` key into `ANTHROPIC_AUTH_TOKEN`, Anthropic's API will return `401 Invalid bearer token` because `sk-ant-*` keys are rejected over the Bearer header. The fix is a one-line env var swap — move the key to `ANTHROPIC_API_KEY`. Recent `my-cli` builds detect this exact shape (401 + `sk-ant-*` in the Bearer slot) and append a hint to the error message pointing at the fix.

**If you meant a different provider:** if `my-cli` reports missing Anthropic credentials but you already have `OPENAI_API_KEY`, `XAI_API_KEY`, or `DASHSCOPE_API_KEY` exported, you most likely forgot to prefix the model name with the provider's routing prefix. Use `--model openai/gpt-4.1-mini` (OpenAI-compat / OpenRouter / Ollama), `--model grok` (xAI), or `--model qwen-plus` (DashScope) and the prefix router will select the right backend regardless of the ambient credentials. The error message now includes a hint that names the detected env var.
...
cd rust
./target/debug/my-cli --model "claude-sonnet-4-6" prompt "reply with the word ready"
```
...
cd rust
./target/debug/my-cli --model "qwen2.5-coder" prompt "reply with the word ready"
```
...
cd rust
./target/debug/my-cli --model "llama3.2" prompt "summarize this repository in one sentence"
```
...
cd rust
./target/debug/my-cli --model "openai/gpt-4.1-mini" prompt "summarize this repository in one sentence"
```
...
cd rust
./target/debug/my-cli --model "qwen/qwen-max" prompt "hello"
# or bare:
./target/debug/my-cli --model "qwen-plus" prompt "hello"
```
...
`my-cli` has three built-in provider backends. The provider is selected automatically based on the model name, falling back to whichever credential is present in the environment.
...
3. Otherwise, `my-cli` checks which credential is set: `ANTHROPIC_API_KEY`/`ANTHROPIC_AUTH_TOKEN` first, then `OPENAI_API_KEY`, then `XAI_API_KEY`.
...
The name "codex" appears in the My CLI ecosystem but it does **not** refer to OpenAI Codex (the code-generation model). Here is what it means in this project:

- **`oh-my-codex` (OmX)** is the workflow and plugin layer that sits on top of `my-cli`. It provides planning modes, parallel multi-agent execution, notification routing, and other automation features. See [PHILOSOPHY.md](./PHILOSOPHY.md) and the [oh-my-codex repo](https://github.com/Yeachan-Heo/oh-my-codex).
- **`.codex/` directories** (e.g. `.codex/skills`, `.codex/agents`, `.codex/commands`) are legacy lookup paths that `my-cli` still scans alongside the primary `.claw/` directories.
- **`CODEX_HOME`** is an optional environment variable that points to a custom root for user-level skill and command lookups.

`my-cli` does **not** support OpenAI Codex sessions, the Codex CLI, or Codex session import/export. If you need to use OpenAI models (like GPT-4.1), configure the OpenAI-compatible provider as shown above in the [OpenAI-compatible endpoint](#openai-compatible-endpoint) and [OpenRouter](#openrouter) sections.

## HTTP proxy support

`my-cli` honours the standard `HTTP_PROXY`, `HTTPS_PROXY`, and `NO_PROXY` environment variables (both upper- and lower-case spellings are accepted) when issuing outbound requests to Anthropic, OpenAI-, and xAI-compatible endpoints. Set them before launching the CLI and the underlying `reqwest` client will be configured automatically.
...
cd rust
./target/debug/my-cli prompt "hello via the corporate proxy"
```
...
- If a proxy URL cannot be parsed, `my-cli` falls back to a direct (no-proxy) client so existing workflows keep working; double-check the URL if you expected the request to be tunnelled.

## Common operational commands

```bash
cd rust
./target/debug/my-cli status
./target/debug/my-cli sandbox
./target/debug/my-cli agents
./target/debug/my-cli mcp
./target/debug/my-cli skills
./target/debug/my-cli system-prompt --cwd .. --date 2026-04-04
```
...
./target/debug/my-cli --resume latest /status /diff
```
...
1. `~/.mycli/settings.json` (override the directory with `MYCLI_CONFIG_HOME`)
